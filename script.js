...
//Starting conditions
let scores, currentScore, activerPlayer, playing;
...
btnHold.addEventListener('click', function () {
  if (playing) {
    ...
    scores[activerPlayer] += currentScore;
    document.getElementById(`score--${activerPlayer}`).textContent =
      scores[activerPlayer];

    // 2.Check score's score is >= 50
    if (scores[activerPlayer] >= 50) {
      playing = false;
      diceEl.classList.add('hidden');
      document
